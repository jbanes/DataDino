 ...
 *
 * @author Slava Pestov
 * @version $Id: InputHandler.java,v 1.2 2002/12/08 09:16:20 jbanes Exp $
 */
public abstract class InputHandler extends KeyAdapter
{
	...
	public static final ActionListener REMOVE_TAB = new remove_tab();
        ...
        public void actionPerformed(ActionEvent evt)
        {
            ...
            if(textArea.getSelectionEnd()-textArea.getSelectionStart() > 0)
            {
                ...
                try
                {
                    ...
                    {
                        textArea.getDocument().insertString(textArea.getLineStartOffset(i), tab, null);
                    }
                }
            ...
            {
                if(spaces) textArea.overwriteSetSelectedText("    ");
                else textArea.overwriteSetSelectedText("\t");
            }
        }
	}

    public static class remove_tab implements ActionListener
    {
        public boolean spaces = true;
        ...
        public void actionPerformed(ActionEvent evt)
        {
            ...
            try
            {
                for(int i=startLine; i<=endLine; i++)
                {
                    text = textArea.getDocument().getText(textArea.getLineStartOffset(i), Math.min((spaces ? 4 : 1), textArea.getLineEndOffset(i)-textArea.getLineStartOffset(i)));
                    delete = 0;
                    
                    for(int j=0; j<(spaces ? 4 : 1) && text.charAt(j) == tab.charAt(0); j++) delete++;
                    
                    if(delete > 0) textArea.getDocument().remove(textArea.getLineStartOffset(i), delete);
                    else if(text.charAt(0) == '\t') textArea.getDocument().remove(textArea.getLineStartOffset(i), 1);
                }
            }
            catch(BadLocationException e)
            {
